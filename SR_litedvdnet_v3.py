...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        return x + 0.1*self.conv2(self.relu(self.conv1(x)))

class SRHead(nn.Module):

    def __init__(self, scale=2, nf=96):
    ...
    def forward(self, x):
        ...
        # shallow features
        shallow_feat = self.head(x)

        # deep residual features
        deep_feat = self.body(shallow_feat)

        # feature fusion
        deep_feat = self.fusion(deep_feat)

        # global residual skip
        deep_feat = deep_feat + shallow_feat

        # reconstruction
        out = self.tail(deep_feat)

        return out

class SR_LiteDVDNet_v3(nn.Module):
    ...
    def __init__(self, num_input_frames=5, inference_mode='Basic', interm_ch=30, simple_cv=False,
        ...
        super(SR_LiteDVDNet_v3, self).__init__()
        ...
        if self.use_sr:
            self.sr_head = SRHead(scale=scale, nf=96)
    ...
    def forward_basic(self, x, noise_map):
        ...
        # Second stage (denoised output)
        den = self.temp2([x20, x21, x22], noise_map)
